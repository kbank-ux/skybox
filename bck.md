	...
	<style lang="scss">
		* {
		...
		#pageTitle {
			...
			left: 50%;                 /* 수평 중앙 정렬을 위해 left:50% + transform */
			transform: translateX(-50%);
			color: white;              /* 글씨 색상 흰색 */
			font-size: 36px;           /* 글씨 크기 크게 */
			font-weight: bold;         /* 두껍게 */
			z-index: 9999;             /* canvas보다 위에 표시 */
			margin: 0;
			padding: 0;
			text-align: center;
		}
...
<script>
	...
	var playerReady = false; // 플레이어 준비 여부 추적용 변수

	window.onYouTubeIframeAPIReady = function() {
		player = new YT.Player('player', {
			height: '0',
			width: '0',
			videoId: '9S-J4-H0XNM', // 재생하고 싶은 유튜브 영상의 ID
			playerVars: {
			autoplay: 0,
			controls: 0,
			showinfo: 0,
			loop: 1,
			playlist: '9S-J4-H0XNM', //  vxdrkrRxiz8   loop를 위해 동일한 ID를 playlist에도 넣음
			mute: 0
			},
			events: {
	...
	// 		height: '0',
	// 		width: '0',
	// 		videoId: 'vxdrkrRxiz8', // 재생하고 싶은 유튜브 영상의 ID
	// 		playerVars: {
	// 		autoplay: 0,
	// 		controls: 0,
	// 		showinfo: 0,
	// 		loop: 1,
	// 		playlist: 'vxdrkrRxiz8', //  vxdrkrRxiz8   loop를 위해 동일한 ID를 playlist에도 넣음
	// 		mute: 0
	// 		},
	// 		events: {
	// 		'onReady': onPlayerReady,
	...
	function onPlayerReady(event) {
		...
		playerReady = true; 
		// 이 시점 이후부터 player.playVideo() 사용 가능
	}

	function onPlayerStateChange(event) {
	// 플레이 상태 변경 시 할 일 (필요없으면 비워둔다)
	}

	// 시작 버튼 클릭 시 호출할 함수
	window.startBackgroundMusic = function() {
		console.log('Start button clicked1');
		if (playerReady && player && player.playVideo) {
			console.log('Start button clicked2');
			player.unMute();
			player.playVideo();
			console.log('Start button clicked3');
		} else {
...
<script type="text/javascript">
	...
	function initialize() {
		...
		document.querySelector('#btnStart').addEventListener('click', () => {
			...
			console.log('Start button clickeda');
			window.startBackgroundMusic(); 
			console.log('Start button clickedb');
			startCountdown(() => {
				// 카운트다운 완료 후 실행할 내용
...
<h1 id="pageTitle">12월 소통미팅, 근데 이제 럭키드로우를 곁들인</h1>
...
<div class="copyright"></div> 원본 : lazygyu</a></div>
</body>
</html>
